'''In this problem, the task is to read a code of a product 1, the number of units of product 1, the price for
 one unit of product 1, the code of a product 2, the number of units of product 2 and the price for one unit of
...
a = input().split(" ")[1:] # input three values and use to split() and three values of string.
b = input().split(" ")[1:] # first line same
total = (int(a[0])*float(a[1]))+(int(b[0])*float(b[1])) # convert list string to int and float numbers.
print(f"VALOR A PAGAR: R$ {total:.2f}") # output this backspace and colon and R$ use
